...
const obsBtn = document.querySelector("#obs");
const traumaBtn = document.querySelector("#trauma");
...
//^^ start / begin subject selection ^^//
const startSection = document.querySelector(".start-landing");
...
//^^ A switch statement to match subject with corresponding questions - first if <i> tag is clicked there's no id //^^ only two options can be selected span text or icon //
    //^^ if target has an id then check if it's span's id else check the icon's parentelement's id for subject //
    ...
    if (e.target.id === "") {
      ...
      switch(e.target.id) {
        ...
        case "cross":
          ...
          break;
        case "family":
          console.log("spant text - family / Obs was selected");
          start(pediatrics);
          setQA(pediatrics);
          break;
        case "emergency":
...
//**global current question index - for updating  */
let indx = 0;
let totalCorrect = 0;
let subjectLength = 0;
...
//** removes q/a section */
function end() {
  const displayCorrectIncorrect = document.createElement('p');
  const backButton = document.createElement('button');

  
  backButton.textContent = `Back To Subjects`;
  displayCorrectIncorrect.classList.add('show-correct');
  ...
  displayCorrectIncorrect.textContent = `Correct: ${totalCorrect}, Incorrect: ${totalIncorrect}`;
  questionTxt.style.gridRow = 2;
  questionTxt.style.alignText = "center";
  choiceSection.style.display = "none";

  questionSection.after(displayCorrectIncorrect);
  displayCorrectIncorrect.after(backButton);
